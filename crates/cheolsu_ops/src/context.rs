...
fn bounded_push<T>(deque: &Mutex<VecDeque<T>>, item: T, max: usize) {
...
impl OpsStore {
    ...
    pub fn push_transaction(&self, info: RequestInfo) {
        bounded_push(&self.transactions, info, self.max_transactions);
    }
    ...
    pub fn push_sse_event(&self, event: SseEventInfo) {
        bounded_push(&self.sse_events, event, self.max_sse_events);
    }
...
mod tests {
    ...
    fn with_config_custom_limits() {
        let store = OpsStore::with_config(StoreConfig {
            max_transactions: 5,
            max_ws_messages: 10,
            max_sse_events: 10,
        });
