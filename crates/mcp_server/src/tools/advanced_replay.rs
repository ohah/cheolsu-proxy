...
impl CheolsuMcpServer {
    ...
    pub(crate) async fn replay_sequence(
        ...
        if p.transaction_ids.is_empty() {
            return tool_error("At least one transaction ID is required.");
        }
    ...
    pub(crate) async fn advanced_repeat(
        ...
        if iterations == 0 {
            return tool_error("iterations must be greater than 0.");
        }
        if iterations > 1000 {
            return tool_error("iterations must be 1000 or less.");
        }
        ...
        for _ in 0..iterations {
            ...
            let handle = tokio::spawn(async move {
                ...
                match result {
                    Ok(resp) => {
                        guard.push((true, resp.status().as_u16(), elapsed));
                    }
                    Err(_) => {
                        guard.push((false, 0u16, elapsed));
                    }
                }
            });
            handles.push(handle);
