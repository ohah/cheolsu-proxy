...
export const getFileNameFromUrl = (uri: string): string | null => {
  try {
    ...
    // URL이 아닌 경우 (예: CONNECT host:port)
    return null;
  }
};
...
export const extractBinaryFileInfo = (
  ...
  // MIME 타입에서 파라미터 제거 (예: "image/png; charset=utf-8" → "image/png")
  const mimeType = contentType.split(";")[0].trim();

  // 확장자 결정: file_path → Content-Type → DataType 기본값
  let fileExtension = "";
  if (filePath) {
  ...
  // 파일 이름 결정: Content-Disposition → URL → 기본값
