...
pub fn check_and_cleanup_stale_lock() -> bool {
    ...
    // PID is still alive — lock is valid
    false
}
...
async fn daemon_main(port: u16, host: String) -> i32 {
    ...
    // Broadcast channel for proxy events → UDS clients
...
async fn run_proxy(
    ...
    // Fan-out HTTP events to broadcast channel
    ...
    // Fan-out tunnel events to broadcast channel
...
mod tests {
    ...
    fn test_stale_lock_with_dead_pid() {
        ...
        let info = ProxyLockInfo {
            pid: 4_000_000, // Very unlikely to be a real PID
            port: 8100,
        ...
        // Verify PID is dead using the same logic
        use nix::sys::signal::kill;
        use nix::unistd::Pid;
    ...
    // --- Protocol Newline Delimiter Tests ---
    ...
    fn test_newline_delimited_protocol_multiple_messages() {
        ...
        // Simulate writing multiple messages with newline delimiter
