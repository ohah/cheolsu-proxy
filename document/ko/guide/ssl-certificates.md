...
이 과정에서 Cheolsu Proxy의 CA 인증서가 macOS Keychain에 추가됩니다.

### 2단계: 인증서 신뢰 설정

인증서를 설치한 것만으로는 충분하지 않습니다. macOS에서 해당 인증서를 **명시적으로 신뢰**해야 합니다.

1. **Keychain Access** 앱을 엽니다 (Spotlight에서 "Keychain Access" 검색)
2. **시스템** 또는 **로그인** 키체인에서 "Cheolsu Proxy" 인증서를 찾습니다
3. 인증서를 **더블클릭**합니다
4. **신뢰** 섹션을 펼칩니다
5. **"이 인증서 사용 시"** 항목을 **"항상 신뢰"**로 변경합니다
6. 창을 닫고 시스템 비밀번호를 입력하여 변경을 확인합니다

> 설정 후 브라우저를 재시작해야 변경사항이 적용됩니다.
...
CA 인증서가 만료되었거나 보안상의 이유로 인증서를 새로 생성해야 하는 경우, 기존 인증서를 삭제한 후 다시 설치할 수 있습니다.

1. **Keychain Access**에서 기존 Cheolsu Proxy 인증서를 삭제합니다
2. Cheolsu Proxy 앱의 **설정** → **인증서**에서 **"인증서 설치"**를 다시 실행합니다
3. 새 인증서에 대해 "항상 신뢰" 설정을 다시 적용합니다
4. 브라우저를 재시작합니다
