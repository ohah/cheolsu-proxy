...
| 클라이언트 트래픽 분리 | IP 태깅, 사용자 태그/라벨, Basic Auth 사용자명 추출, 필터링 | Charles 동등                               |
...
- ✅ 클라이언트 IP별 자동 태깅 (`client_addr` 필드)
- ✅ 사용자 정의 태그/라벨 부여 (설정 > Network > Client Tags)
- ✅ 태그별 필터링 (`client="태그명"`, `client="IP"`, `client!="제외"`)
- ✅ 프록시 접속 인증 사용자명 태깅 (`proxy_auth_user` 필드, Basic Auth 시 자동 추출)
...
구현 완료. OpenAPI 스펙 로드, 실시간 트래픽 검증, 위반 사항 보고 지원. 자세한 내용은 [Contract Testing](./contract-testing.md) 문서를 참고하세요.
