import { describe, test, expect } from "bun:test";

import { generateVitestCode } from "./vitest";
import type { HttpTransaction } from "@/entities/proxy/model/types";
import type { DataType } from "@/entities/proxy/model/data-type";

const createTransaction = (
  overrides: {
    method?: string;
    uri?: string;
    headers?: Record<string, string>;
    body?: string | null;
    data_type?: DataType;
    responseStatus?: number;
    responseBodyJson?: unknown;
  } = {},
): HttpTransaction => {
  const bodyText = overrides.body ?? null;
  const bodyBytes = bodyText ? new TextEncoder().encode(bodyText) : null;

  return {
    request: {
      method: overrides.method ?? "GET",
      uri: overrides.uri ?? "https://example.com/api/users",
      version: "HTTP/1.1",
      headers: overrides.headers ?? {},
      body: bodyBytes,
      time: Date.now(),
      id: "test-id",
      data_type: overrides.data_type ?? "Text",
      body_size: bodyBytes ? bodyBytes.length : 0,
    },
    response: {
      status: overrides.responseStatus ?? 200,
      version: "HTTP/1.1",
      headers: {},
      body: null,
      time: Date.now(),
      id: "test-id",
      data_type: "Json",
      body_size: 0,
      body_json: overrides.responseBodyJson ?? undefined,
    },
  };
};

const createEmptyTransaction = (): HttpTransaction => ({
  request: null,
  response: null,
});

describe("generateVitestCode", () => {
  test("request가 없으면 기본 코드 반환", () => {
    const result = generateVitestCode(createEmptyTransaction());
    expect(result).toContain('import { describe, it, expect } from "vitest"');
    expect(result).toContain("http://localhost");
  });

  test("GET 요청 기본 테스트 생성", () => {
    const result = generateVitestCode(createTransaction());
    expect(result).toContain('describe("GET /api/users"');
    expect(result).toContain("expect(response.status).toBe(200)");
    expect(result).toContain("await fetch");
  });

  test("POST + JSON body 포함", () => {
    const result = generateVitestCode(
      createTransaction({
        method: "POST",
        body: '{"name":"test"}',
        data_type: "Json",
      }),
    );
    expect(result).toContain('method: "POST"');
    expect(result).toContain("JSON.stringify(");
  });

  test("응답 JSON 스키마 assertion 생성", () => {
    const result = generateVitestCode(
      createTransaction({
        responseBodyJson: { id: 1, name: "test", active: true },
      }),
    );
    expect(result).toContain("await response.json()");
    expect(result).toContain("expect.any(Number)");
    expect(result).toContain("expect.any(String)");
    expect(result).toContain("expect.any(Boolean)");
  });

  test("응답 없는 경우 text assertion", () => {
    const tx: HttpTransaction = {
      request: {
        method: "GET",
        uri: "https://example.com/api",
        version: "HTTP/1.1",
        headers: {},
        body: null,
        time: Date.now(),
        id: "test-id",
        data_type: "Text",
        body_size: 0,
      },
      response: null,
    };
    const result = generateVitestCode(tx);
    expect(result).toContain("expect(response.status).toBe(200)");
    expect(result).toContain("await response.text()");
  });

  test("404 응답 상태 코드 반영", () => {
    const result = generateVitestCode(createTransaction({ responseStatus: 404 }));
    expect(result).toContain("expect(response.status).toBe(404)");
    expect(result).toContain('should return 404"');
  });

  test("헤더 포함", () => {
    const result = generateVitestCode(
      createTransaction({
        headers: { Authorization: "Bearer token123" },
      }),
    );
    expect(result).toContain("headers:");
    expect(result).toContain("Authorization");
    expect(result).toContain("Bearer token123");
  });
});
