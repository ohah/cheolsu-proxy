...
This repo (ohah/cheolsu-proxy) uses the **ohah** GitHub account for push and PR.
...
- **SSH remote**: So that `git push` authenticates as ohah, origin must use the SSH host `github.com-private` (see `~/.ssh/config`). Before any push, ensure origin is set to:
  ```bash
  git remote set-url origin git@github.com-private:ohah/cheolsu-proxy.git
  ```
...
2. **SSH remote**: Ensure origin is `git@github.com-private:ohah/cheolsu-proxy.git` so push uses ohah's SSH key; if not, run `git remote set-url origin git@github.com-private:ohah/cheolsu-proxy.git`.
