...
This repo (ohah/cheolsu-proxy) uses the **ohah** GitHub account for posting reviews.
...
4. **Write the AI review (summary + line suggestions)**
   ...
   - **Line-level suggestions**: For each place that needs a change, prepare an inline comment with:
     - **path**: Repo-root-relative path (e.g. `crates/proxy_v2_models/src/data_type.rs`)
   ...
   - **5-a. When there is at least one inline suggestion**
     ...
     - Example payload file (`review-payload.json`):
       ````json
       {
         ...
         "comments": [
           {
             "path": "crates/proxy_v2_models/src/data_type.rs",
             "line": 42,
             "side": "RIGHT",
             "body": "Consider adding error handling for invalid encoding.\\n\\n```suggestion\\n  if encoding.is_empty() { return Err(...); }\\n```"
           }
         ]
       }
       ````
     - Commands:
       ...
       gh api repos/ohah/cheolsu-proxy/pulls/$(gh pr view --json number -q .number)/reviews --input review-payload.json
       ```
...
- Run from the repo root with `gh` authenticated. This repo (ohah/cheolsu-proxy): use ohah for posting reviews; switch gh before submit and restore after (see "gh account for this repo" and step 1, step 6).
